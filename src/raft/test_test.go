...
func TestReElection2A(t *testing.T) {
	...
	cfg.checkOneLeader()
	log.Printf("disconnected leader passed====")
	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	log.Printf("old leader rejoins passed====")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	log.Printf("no quorum passed====")

	// if a quorum arises, it should elect a leader.
	log.Printf("a quorum: leader2=%v", leader2)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	log.Printf("a quorum arises passed====")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	log.Printf("rejoin of last node passed====")
